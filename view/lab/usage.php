...
FROM lot_lab_report_retail_cache
WHERE lab_report_id LIKE 'WAATTEST%'
...
SELECT license_retail, count(id) AS lot_count
FROM lot_lab_report_retail_cache
WHERE lab_report_id NOT LIKE 'WAATTEST%'
GROUP BY license_retail
ORDER BY lot_count DESC
LIMIT 25
SQL;
...
$sql = <<<SQL
SELECT lab_report_id, count(id) AS lot_count
FROM lot_lab_report_retail_cache
WHERE lab_report_id LIKE 'WAATTEST%'
GROUP BY lab_report_id
ORDER BY lot_count DESC
LIMIT 25
SQL;
$res0 = _select_via_cache($dbc, $sql, $arg);
...
// select count(id) from lot_lab_report_retail_cache where length(meta->>'lab_report_id') = 17;
// select count(id) from lot_lab_report_retail_cache where length(meta->>'lab_report_id') = 16;

// SELECT count(id) AS c, meta->>'lab_report_id' AS lab_report_id, min(created_at) AS min_date, max(created_at) AS max_date
// FROM lot_lab_report_retail_cache
// GROUP BY meta->>'lab_report_id'
// ORDER BY 1 DESC;

// SELECT id, license_id, meta->>'external_id' FROM lot_lab_report_retail_cache WHERE meta->>'lab_report_id' = 'WAL21.LR1243A' ORDER BY created_at DESC;
